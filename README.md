## v2.8.8.10 · COLUMN LEAD COPY HEIGHT SYNC

### v2.8.8.10 변경사항
- 기준본: v2.8.8.9 · HOME CONTENT ORDER
- 메인 순서(NOW RANK → IT’S ME → COLUMN → SURVEY → COMMUNITY) 유지
- 기존 COLUMN 대표 이미지의 크기·비율·crop·소스는 변경하지 않음
- 첫 번째 큰 COLUMN에서 오른쪽 글 영역만 이미지와 동일한 grid-track 높이로 stretch
- 모바일 기존 세로 배치 유지
- 관리자 화면 버전: v2.8.8.10 · COLUMN LEAD COPY HEIGHT SYNC
...
- `/api/news?action=image` 응답을 브라우저/CDN 모두 `no-store`로 고정해 이전 이미지 재사용 차단
- 관리자 사진 저장 직후 서버가 반환한 최신 게시물을 HOME / COLUMN 목록 / 상세 렌더 데이터에 즉시 반영
- COLUMN 목록 재조회도 `cache: no-store`로 강제
- 기존 v2.8.8.3 검색결과 페이지 / COLUMN 서버 단일화 / 폰트 크기 유지
- 관리자 화면 버전: v2.8.8.4 · COLUMN IMAGE HARD RESET
