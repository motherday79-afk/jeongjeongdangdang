# 정참시 v2.7.3 · DETAIL TRUST / 8 AXIS / PHOTO MASTER 562 / COLUMN IMAGE FIX
...
## v2.7.3 추가
- COLUMN 대표사진 교체 시 imageVersion을 발급해 immutable 브라우저/CDN 캐시를 즉시 무효화합니다.
- 실시간 정치키워드는 실제 행 위치를 계산해 정확히 2줄까지만 노출하고 더보기에서 전체를 펼칩니다.
- PC 우측 LIVE DESK는 sticky로 따라오며 메인 레일 하단에서 자연스럽게 멈춥니다.
- 메인 NOW Rank는 순위/정치인/정당/지역·직위/변동만 노출하고 핵심 폰트를 상향했습니다.
- 상세 NOW 등급은 공개, 정확한 NOW 지수와 8축 숫자는 PLUS 이상에게 노출합니다.
- 모든 상세 레이더를 고정 8축으로 정규화하고 420 viewBox 라벨 링으로 교체했습니다.
- 개발자 설명형 RECORD 카드를 제거하고 사용자 중심 ‘지금 이 인물’ 요약 + 접이식 데이터 출처로 변경했습니다.
- PHOTO MASTER 구축은 실패 인물을 repair 검색으로 자동 재시도합니다.
