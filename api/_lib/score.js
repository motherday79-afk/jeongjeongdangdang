...
const {credentialStatus}=require('./naver_common');
...
function automaticCentralityDetail(name,clusters=[]){
  const rows=[];for(const c of clusters){const mc=Number(c.members?.[name]||0);if(!mc)continue;const contribution=c.strength*Math.sqrt(mc/Math.max(1,c.articles)),coMembers=Object.entries(c.members||{}).filter(([n])=>n!==name).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([n,count])=>({name:n,count}));rows.push({term:c.term,articles:c.articles,sources:c.sources,strength:c.strength,memberMentions:mc,contribution:round1(contribution),coMembers});}
  rows.sort((a,b)=>b.contribution-a.contribution);const vals=rows.map(x=>x.contribution),score=clamp((vals[0]||0)+.35*(vals[1]||0)+.15*(vals[2]||0));return {score:round1(score),clusters:rows.slice(0,4)};
}
function automaticCentrality(name,clusters=[]){return automaticCentralityDetail(name,clusters).score;}
function globalEventCentralityDetail(name,globalNews={}){
  ...
  const ec=Number(s.eventCount||0),et=Number(s.eventTitleCount||0),es=Number(s.eventSources||0),bc=Number(s.broadCount||0),bs=Number(s.broadSources||0),bt=Number(s.broadTitleCount||0);
  ...
  const broadPresence=.55*logisticCount(bc,8)+.30*logisticCount(bs,5)+.15*logisticCount(bt,5),score=clamp(Math.max(eventPresence,.42*broadPresence));
  return {score:round1(score),eventCount:ec,eventTitleCount:et,eventSources:es,broadCount:bc,broadTitleCount:bt,broadSources:bs,eventCorpusArticles:Number(globalNews.eventArticleCount||0),eventCorpusSources:Number(globalNews.eventSourceCount||0),eventPresence:round1(eventPresence),broadPresence:round1(broadPresence)};
}
function globalEventCentrality(name,globalNews={}){return globalEventCentralityDetail(name,globalNews).score;}
...
function computeMember(signal={},traffic={},eventSignals={},previousSignal={}){
  ...
  const searchParts=[
    {key:'naverTrend',value:naverTrend,weight:.52,availability:availability(ch.naverSearchTrend)},
    {key:'googleTrend',value:googleTrend,weight:.10,availability:availability(ch.googleTrends)},
    {key:'wiki',value:wiki.score,weight:.14,availability:availability(ch.wiki)},
    {key:'internal',value:internal.score,weight:.08,availability:(traffic.count6||traffic.baseline7d)?1:0},
    {key:'searchSurface',value:searchSurface,weight:.16,availability:Math.max(availability(ch.naverSurface),availability(ch.daumSurface),availability(ch.naverWebApi),availability(ch.wiki))}
  ],searchBreakdown=weightedBreakdown(searchParts),searchInterest=searchBreakdown.total;
  ...
  const mEvent=manualEvent(effectiveSignal),autoEvent=Number(eventSignals.auto||0),globalEvent=Number(eventSignals.global||0);
  const eventBlend=globalEvent>0?{mode:'global-blend',global:.58*globalEvent,auto:.25*autoEvent,manual:.17*mEvent,total:clamp(.58*globalEvent+.25*autoEvent+.17*mEvent)}:{mode:'auto-vs-manual',autoCandidate:autoEvent,manualBlendCandidate:.65*mEvent+.35*autoEvent,total:clamp(Math.max(autoEvent,.65*mEvent+.35*autoEvent))},event=eventBlend.total;
...
function computeSnapshot(roster,signals,context={},previous=null,trafficSignals={}){
  ...
  const comparisonCompatible=!previous||previous.modelVersion==='common-sensor-v1.1';
  let scored=active.map(m=>{const sig=signals[m.name]||{},traffic=trafficSignals[m.name]||{},autoD=automaticCentralityDetail(m.name,clusters),globD=globalEventCentralityDetail(m.name,globalNews),prevSig=previous?.members?.find?.(p=>p.name===m.name)?.signal||{},c=computeMember(sig,traffic,{auto:autoD.score,global:globD.score,autoEvidence:autoD,globalEvidence:globD},prevSig);sig.channels=c.effectiveChannels;const ch=sig.channels||{},tiebreak=(ch.news?.count24||0)*1e-5+(ch.news?.sources24||0)*1e-6-m.id*1e-10;return {member:m,signal:sig,traffic,components:c,sortScore:c.raw+tiebreak};}).sort((a,b)=>b.sortScore-a.sortScore);
  const pools={
  ...
  const okState=s=>['READY','OK','OK_ROBOTS_DISALLOW','ADVISORY_DISALLOW','ADVISORY_UNKNOWN','DEGRADED'].includes(s||''),naverCfg=credentialStatus(),configured={googleNews:true,naverSearchTrend:naverCfg.configured,naverCommon:naverCfg.configured,googleTrends:true,wikimedia:true,naverHtml:okState(context.sourceHealth?.naverHtml?.state),daumHtml:okState(context.sourceHealth?.daumHtml?.state),youtubeHtml:okState(context.sourceHealth?.youtubeHtml?.state),naverNews:naverCfg.configured,kakao:Boolean(process.env.KAKAO_REST_API_KEY),youtube:Boolean(process.env.YOUTUBE_API_KEY),x:Boolean(process.env.X_BEARER_TOKEN),internal:true,globalPolitics:true};
  ...
  const sourceSummary=Object.entries(configured).filter(([,on])=>on).map(([k])=>k);return {version:'NOW Rank v1.9 Sensor Recovery',modelVersion:'common-sensor-v1.1',displayScale:'relative-v1.6.1',comparisonCompatible,timestamp:new Date().toISOString(),cadenceHours:6,event:{title:context.eventTitle||'',keywords:context.eventKeywords||[]},configuredSources:configured,sourceSummary,sourceHealth:context.sourceHealth||{},globalNews:{eventArticles:globalNews.eventArticleCount||0,eventSources:globalNews.eventSourceCount||0,broadArticles:globalNews.broadArticleCount||0,broadSources:globalNews.broadSourceCount||0,warnings:globalNews.warnings||[]},detectedIssues:clusters.slice(0,5),quality,members:scored};
}
module.exports={computeSnapshot,computeMember,buildIssueClusters,channelMomentum,grade,clamp,globalEventCentrality,globalEventCentralityDetail,automaticCentralityDetail,relativeDisplay};
