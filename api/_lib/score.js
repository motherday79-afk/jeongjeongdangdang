...
function computeMember(signal={},traffic={},eventSignals={},previousSignal={}){
  ...
  let raw=blendAvailable([{value:searchInterest,weight:.28,availability:searchAvail},{value:news.score,weight:.22,availability:newsAvail},{value:portalSocial,weight:.18,availability:socialAvail},{value:event,weight:.24,availability:1},{value:freshAccel,weight:.08,availability:freshAvail}]);
  const families=sourceFamilyCount(effectiveSignal,traffic),cap=[28,45,66,82,92,97,100][Math.min(6,families)]||28;raw=Math.min(raw,cap);
  ...
  return {raw,searchInterest,news:news.score,portalSocial,event,freshAccel,families,evidenceConfidence,effectiveChannels:ch,collection:{observedChannels,missingChannels,carriedChannels},details:{news,wiki,internal,naverSurface,daumSurface,naverView,daumBlog,naverBlogApi,naverCafeApi,naverWebApi,naverTrend,googleTrend,web,blog,cafe,kvideo,yt,yth,xx,mEvent,autoEvent,globalEvent}};
}
...
function computeSnapshot(roster,signals,context={},previous=null,trafficSignals={}){
  ...
  scored=scored.map((x,i)=>{const display=relativeDisplay(x.components.raw,pools.score,{floor:12,top:98,percentileWeight:.76});const prev=previous?.members?.find?.(p=>p.name===x.member.name),change6h=Number.isFinite(prev?.rank)?prev.rank-(i+1):null,metrics=[makeMetric('검색·프로필 급상승',relativeDisplay(x.components.searchInterest,pools.search)),makeMetric('뉴스 급상승',relativeDisplay(x.components.news,pools.news)),makeMetric('포털·SNS 확산',relativeDisplay(x.components.portalSocial,pools.social)),makeMetric('이슈 중심성',relativeDisplay(x.components.event,pools.event)),makeMetric('신선도·가속도',relativeDisplay(x.components.freshAccel,pools.fresh))],channels=x.signal.channels||{},sourceBadges=[];
    ...
    return {id:x.member.id,name:x.member.name,party:x.member.party,region:x.member.region,constituency:x.member.constituency,rank:i+1,score:display,rawScore:round1(x.components.raw),grade:grade(display),change6h,metrics,sourceCount:x.components.families,evidenceConfidence:round1(x.components.evidenceConfidence),collection:x.components.collection,sourceBadges,eventLabel:context.eventTitle||'현재 주요 정치 이슈',latest:channels.news?.latest?new Date(channels.news.latest).toISOString():null,signal:{count6:channels.news?.count6||0,count24:channels.news?.count24||0,count7d:channels.news?.count7d||0,sources6:channels.news?.sources6||0,event6:channels.news?.event6||0,headlines:channels.news?.headlines||[],channels,traffic:x.traffic,autoEvent:round1(x.components.details.autoEvent),globalEvent:round1(x.components.details.globalEvent),manualEvent:round1(x.components.details.mEvent)}};});
  ...
  const sourceSummary=Object.entries(configured).filter(([,on])=>on).map(([k])=>k);return {version:'NOW Rank v1.7 Common Sensor',displayScale:'relative-v1.6.1',timestamp:new Date().toISOString(),cadenceHours:6,event:{title:context.eventTitle||'',keywords:context.eventKeywords||[]},configuredSources:configured,sourceSummary,sourceHealth:context.sourceHealth||{},globalNews:{eventArticles:globalNews.eventArticleCount||0,eventSources:globalNews.eventSourceCount||0,broadArticles:globalNews.broadArticleCount||0,broadSources:globalNews.broadSourceCount||0,warnings:globalNews.warnings||[]},detectedIssues:clusters.slice(0,5),quality,members:scored};
}
