...
function memberKeywords(member){
  ...
  return unique([name,`${name} 의원`,`${name} 국회의원`,party&&`${name} ${party}`,region&&`${name} ${region}`,constituency&&`${name} ${constituency}`]).slice(0,12);
}
...
async function collectSearchSurfaces(member,keywords=[]){
  ...
  const settled=await Promise.all(specs.map(async ([type,path,extra])=>{try{const j=await call(path,{query:member.name,display:100,...extra});return [type,surfaceFromSearch(j,member,type,keywords)];}catch(e){return [type,missing(`naver-${type}-api`,e.message||String(e),'surface')];}}));
...
async function collectTrendBatch(members=[],anchorMember){
  if(!creds()){const out={};for(const m of members)out[m.name]=missing('naver-search-trend','NAVER API HUB credentials not configured');return {signals:out,configured:false};}
  const uniq=new Map([[anchorMember.name,anchorMember]]);for(const m of members)uniq.set(m.name,m);
  const groups=[...uniq.values()].slice(0,5).map(m=>({groupName:m.name,keywords:memberKeywords(m)}));
  ...
  const text=await r.text();if(!r.ok){const out={};for(const m of members)out[m.name]=missing('naver-search-trend',`NAVER trend ${r.status} ${text.slice(0,120)}`);return {signals:out,configured:true,error:`${r.status}`};}
  let j;try{j=JSON.parse(text)}catch(e){const out={};for(const m of members)out[m.name]=missing('naver-search-trend','invalid-json');return {signals:out,configured:true,error:'invalid-json'};}
  const byTitle=new Map((j.results||[]).map(x=>[x.title,x])),anchor=byTitle.get(anchorMember.name);const out={};
  for(const m of members){const result=byTitle.get(m.name);out[m.name]=result&&anchor?trendSummary(result,anchor,m,anchorMember.name):missing('naver-search-trend','member or anchor result missing');}
