...
module.exports=async function(req,res){
  ...
  try{
    if(req.method==='GET'){
      const st=await photoMasterStatus(ids),rows=(st.rows||[]).map(r=>({...r,name:findEntity(r.id)?.name||String(r.id)}));return res.json({ok:true,total:ids.length,masterVersion:MASTER_VERSION,...st,rows});
    }
