...
async function publicPost(p,count,likes,me,repOverride){return {id:p.id,category:p.category,title:p.title,content:p.content,author:p.author,authorId:p.authorId||null,role:p.role,createdAt:p.createdAt,commentCount:Number(count??p.commentCount??0),likeCount:Number(likes??p.likeCount??0),liked:me?.id?Boolean(Number(await cmd(['SISMEMBER',likeKey(p.id),String(me.id)]).catch(()=>0))):false,representativeBadge:repOverride!==undefined?repOverride:(p.authorId?await getRepresentativeBadge(p.authorId).catch(()=>null):null)};}
...
async function listPosts(limit=60,me=null){
  const [rows,countsRaw,likesRaw]=await Promise.all([cmd(['LRANGE',POSTS,'0',String(Math.max(0,Math.min(99,limit-1)))]).catch(()=>[]),cmd(['HGETALL',COMMENT_COUNTS]).catch(()=>null),cmd(['HGETALL',LIKE_COUNTS]).catch(()=>null)]);const counts=parseHash(countsRaw),likes=parseHash(likesRaw),posts=[];for(const x of (Array.isArray(rows)?rows:[])){try{posts.push(JSON.parse(x));}catch(_){}}const reps=await getRepresentativeBadges(posts.map(p=>p.authorId)).catch(()=>({}));return Promise.all(posts.map(p=>publicPost(p,counts[p.id],likes[p.id],me,p.authorId?reps[p.authorId]??null:null)));
}
