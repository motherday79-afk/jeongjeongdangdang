...
async function latestNews(limit=16){
  ...
  return posts.map(p=>{const imageVersion=String(p.imageVersion||p.updatedAt||p.createdAt||'1');return {id:p.id,category:p.category,title:p.title,excerpt:p.excerpt,content:p.content,author:p.author,authorId:p.authorId||null,role:p.role,createdAt:p.createdAt,updatedAt:p.updatedAt||null,imageVersion,hasImage:Boolean(p.hasImage),imageUrl:p.hasImage?`/api/news?action=image&id=${encodeURIComponent(p.id)}&v=${encodeURIComponent(imageVersion)}`:(p.imageUrl||null),representativeBadge:p.authorId&&!String(p.authorId).startsWith('admin:')?reps[p.authorId]??null:null};});
}
