const {resolveLocalPhoto,invalidateLocalPhoto}=require('../lib/local_photo');
...
module.exports=async function(req,res){
  ...
  try{
    let photo=await resolveLocalPhoto(id);if(!photo?.url)return res.status(404).end();
    let got=await fetchImage(photo);
    if(!got){
      await invalidateLocalPhoto(id);
      photo=await resolveLocalPhoto(id,{force:true});
