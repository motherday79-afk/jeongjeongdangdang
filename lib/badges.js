...
const FIXED_BADGES=[
  ...
  {id:'seven_day',category:'생활',name:'7일 연속 시민',icon:'🔥',tier:'STANDARD',desc:'7일 연속 정참시를 찾은 시민',hint:'7일 연속 로그인 방문 기록을 남기세요.',secret:false,condition:a=>streak(a)>=7,progress:a=>prog(streak(a),7)},
  ...
  {id:'explorer',category:'탐험',name:'정참시 탐험가',icon:'🧭',tier:'STANDARD',desc:'정참시의 핵심 공간을 모두 둘러본 시민',hint:'NOW · 정뮤니티 · IT’S ME · News · 비교분석 · MY를 모두 방문하세요.',secret:false,condition:a=>['ranking','community','itsme','news','compare','my'].every(x=>a.pages?.[x]),progress:a=>{const n=['ranking','community','itsme','news','compare','my'].filter(x=>a.pages?.[x]).length;return prog(n,6)}},
  ...
  {id:'column_first',category:'칼럼',name:'첫 칼럼',icon:'🖋️',tier:'STANDARD',desc:'정참시News에 첫 글을 발행한 작가',hint:'PRO 이상 권한으로 정참시News 글을 1개 발행하세요.',secret:false,condition:a=>num(a,'columns')>=1,progress:a=>prog(num(a,'columns'),1)},
  ...
  {id:'news_reader',category:'칼럼',name:'뉴스룸 단골',icon:'📰',tier:'STANDARD',desc:'정참시 뉴스룸을 꾸준히 찾은 독자',hint:'정참시News 화면을 10회 방문하세요.',secret:false,condition:a=>Number(a?.pages?.news||0)>=10,progress:a=>prog(Number(a?.pages?.news||0),10)},
...
const MANUAL_BADGES=[
  {id:'challenge_winner',category:'명예',name:'IT’S ME 시즌 우승자',icon:'🏆',tier:'HONOR',desc:'IT’S ME 시즌에서 최종 우승한 시민',manual:true},
  {id:'citizen_legislator',category:'명예',name:'시민입법가',icon:'⚖️',tier:'HONOR',desc:'정책 제안이 공식 정책 검토 단계에 진입한 시민',manual:true},
  {id:'policy_delivery',category:'명예',name:'국회 전달',icon:'📨',tier:'HONOR',desc:'정참시 정책브리프가 국회·정당 정책조직에 공식 전달된 시민',manual:true},
  {id:'real_policy',category:'LEGACY',name:'REAL POLICY',icon:'🏛️',tier:'LEGACY',desc:'정참시 제안이 실제 정책·법안 논의에 반영된 시민',manual:true},
  {id:'first_discoverer',category:'히든',name:'FIRST DISCOVERER',icon:'🔓',tier:'LEGACY',desc:'새로운 히든배지의 최초 발견자로 기록된 시민',manual:true}
];
...
async function getProfile(userId){
  const [a,cfg]=await Promise.all([getActivity(userId),getConfig()]);let s=await refreshAwards(userId,a);const fixed=FIXED_BADGES.map((b,i)=>{const rec=awardRecordFor(b.id,s),earned=Boolean(rec)&&!s.revoked?.[b.id]&&!isDisabled(cfg,b.id),p=b.progress(a),lockedSecret=b.secret&&!earned;const base=stripFn(b);return {...base,id:lockedSecret?`hidden_${i+1}`:base.id,earned,earnedAt:rec?.at||null,lockedSecret,displayName:lockedSecret?'???':b.name,displayDesc:lockedSecret?'숨겨진 조건을 발견하면 공개됩니다.':b.desc,displayHint:lockedSecret?'조건 비공개':b.hint,progress:p,style:tierStyle(b.tier)}});
  const families=[];
  for(const family of Object.keys(TIER_FAMILIES)){
...
module.exports={BADGE_SCHEMA,FIXED_BADGES,TIER_FAMILIES,MANUAL_BADGES,allCatalog,getActivity,getState,getConfig,saveConfig,getProfile,recordActivity,recordVisit,recordPageView,setCounter,setRepresentative,clearRepresentative,getRepresentativeBadge,getRepresentativeBadges,follow,followState,manualAward,revokeBadge,getSeasonConfig,saveSeasonConfig,clearUserData,tierBadgeId,tierStyle};
