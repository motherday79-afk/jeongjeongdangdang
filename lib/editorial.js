...
async function saveDraft(input,admin){
  ...
  const d={
    ...
    author:type==='news'?'정참시News 편집부':'정참시 운영팀',authorId:`admin:${admin?.id||'admin'}`,role:'ADMIN',
...
async function publishDraft(id,{force=false}={}){
  ...
  try{
    ...
    if(d.type==='community'){
      ...
      const post={id:postId,category:d.category,title:d.title,content:d.content,author:d.author,authorId:d.authorId,role:'ADMIN',createdAt:now,commentCount:0,adminPublished:true};
      ...
      const post={id:postId,category:d.category,title:d.title,excerpt:d.excerpt,content:d.content,author:d.author,authorId:d.authorId,role:'ADMIN',createdAt:now,hasImage:Boolean(image?.data),schemaVersion:1,adminPublished:true};
