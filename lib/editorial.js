...
async function publishDraft(id,{force=false}={}){
  ...
  try{
    ...
    if(d.type==='community'){
      ...
      const post={id:postId,category:d.category,title:d.title,excerpt:d.excerpt,content:d.content,author:d.author,authorId:d.authorId,role:d.role||'ADMIN',createdAt:now,hasImage:Boolean(image?.data),schemaVersion:1,adminPublished:true};
