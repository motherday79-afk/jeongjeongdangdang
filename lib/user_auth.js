...
async function createUser({email,password,nickname,agreeTerms,agreePrivacy,age14}){
  const normalized=normalizeEmail(email);
  if(!validateEmail(normalized)) throw new Error('이메일 형식을 확인해주세요.');
  if(!validatePassword(password)) throw new Error('비밀번호는 10자 이상 128자 이하로 입력해주세요.');
  ...
  const idx=emailIndexKey(normalized);
  const reserved=await cmd(['SET',idx,id,'NX']);
  if(reserved!=='OK') throw new Error('이미 가입된 이메일입니다.');
  try{
    const ts=nowIso();
    const user={
      id,email:normalized,nickname:String(nickname).trim(),role:'FREE',status:'ACTIVE',
...
module.exports={
  ROLES,AGREEMENT_VERSIONS,USER_SET_KEY,userKey,emailIndexKey,normalizeEmail,
  hashPassword,verifyPassword,encryptSensitive,decryptSensitive,publicUser,capabilities,
  getUserById,getUserByEmail,authenticate,requireUser,createUser,saveUser,
  sessionCookie,clearSessionCookie,validateEmail,validateNickname,validatePassword,rateLimit
};
