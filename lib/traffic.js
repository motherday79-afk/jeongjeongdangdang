...
const KST=9*3600000;
const currentRoster=activeRoster();
const nameCounts=currentRoster.reduce((m,x)=>(m[x.name]=(m[x.name]||0)+1,m),{});
...
function resolveMember({memberId,name}){
  if(Number.isFinite(Number(memberId))){
    const m=currentRoster.find(x=>Number(x.id)===Number(memberId));
    if(m && (!name||m.name===name))return m;
  }
  const matches=currentRoster.filter(x=>x.name===String(name||''));
