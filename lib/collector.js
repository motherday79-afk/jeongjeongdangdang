...
const {politicalContext}=require('./naver_common');
const {collectBigKinds,credentials:bigKindsCredentials}=require('./bigkinds');
...
const {titleTerm,searchQualifier}=require('./political_entity');
...
function memberSearchQuery(member,{quoted=true}={}){const name=quoted?`\"${member.name}\"`:member.name;const clue=member?.ambiguousName?disambiguationTerms(member)[0]:'';return [name,titleTerm(member),clue].filter(Boolean).join(' ');}
...
function summarize(items,member,keywords,nowMs,provider,totalCount=null){
  ...
  const summary={count6:recent6.length,count24:recent24.length,count7d:recent7.length,sources6:sources6.size,sources24:sources24.size,event6:event6.length,title6,latest,totalCount,provider,
    ...
    headlineCandidates:recent7.slice(0,12).map(x=>({title:x.title,source:x.source,ts:x.ts,link:x.link,channel:provider}))};
...
async function fetchNaverNews(member){
  ...
  const q=memberSearchQuery(member,{quoted:false});
...
async function fetchGoogleNews(member){
  ...
  const role=titleTerm(member);
  const qualifier=party?`("${role}" OR "${party}")`:`"${role}"`;
  const identity=memberSearchQuery(member);
  const query=`${identity} ${qualifier} when:7d`;
...
async function fetchKakao(member,type){
  ...
  const query=memberSearchQuery(member,{quoted:false});
...
function mergeSummaries(list,provider='news-multi'){
  const valid=list.filter(Boolean),usable=valid.filter(x=>x?.observation?.state!=='MISSING'),heads=dedupe(usable.flatMap(x=>x.headlines||[])),headlineCandidates=dedupe(usable.flatMap(x=>x.headlineCandidates||x.headlines||[]));
  ...
  const summary={count6:usable.reduce((a,x)=>a+Number(x.count6||0),0),count24:usable.reduce((a,x)=>a+Number(x.count24||0),0),count7d:usable.reduce((a,x)=>a+Number(x.count7d||0),0),
    ...
    latest:latest.length?Math.max(...latest):null,provider,headlines:heads.slice(0,12),headlineCandidates:headlineCandidates.slice(0,18),providers:valid.map(x=>x.provider),sourceStates:Object.fromEntries(valid.map(x=>[x.provider,x?.observation?.state||'UNKNOWN']))};
...
async function collectMember(member,keywords=[],globalSignals={}){
  ...
  const [google,naver,keyless,kakaoRows]=await Promise.all([
    safe('Google News',()=>fetchGoogleNews(member),null),safe('NAVER News',()=>fetchNaverNews(member),null),safe('Keyless Surface',()=>collectKeylessSurface(member,keywords,globalSignals),null),kakaoPromise
  ]);
  const googleSummary=google?summarize(google,member,keywords,nowMs,'google-news-rss'):missing('google-news-rss','Google News request failed');
  ...
  const trendSignal=globalSignals?.trends?.signals?.[memberKey(member)]||{provider:'google-trends-rising',score:0,traffic:0,exact:false,latest:null,title:null};
  const channels={news,googleTrends:mark(trendSignal,'google-trends-rising',(trendSignal?.score||0)>0?STATES.OBSERVED:STATES.ZERO)};
  ...
  return {channels,evidenceItems,health:surfaceHealth,warning:warnings.length?warnings.join('; '):null,providers:{googleNews:true,naverNews:Boolean(naver),kakao:Boolean(process.env.KAKAO_REST_API_KEY),googleTrends:true}};
}
...
function memberMapFromCorpus(items,active,eventKeywords=[]){
  const members={};for(const m of active)members[memberKey(m)]={eventCount:0,eventTitleCount:0,eventSources:new Set(),broadCount:0,broadTitleCount:0,broadSources:new Set(),latest:null};
  for(const x of items){
    const txt=`${x.title} ${x.desc||''}`;for(const m of active){if(!politicalContext(m,txt))continue;const k=memberKey(m),st=members[k],isEvent=(x.tags||[]).includes('event')||keywordMatch(txt,eventKeywords);if(isEvent){st.eventCount++;if(x.title.includes(m.name))st.eventTitleCount++;if(x.source)st.eventSources.add(x.source);}else{st.broadCount++;if(x.title.includes(m.name))st.broadTitleCount++;if(x.source)st.broadSources.add(x.source);}st.latest=Math.max(st.latest||0,x.ts||0);}
  }
  const out={};for(const [key,s] of Object.entries(members))out[key]={eventCount:s.eventCount,eventTitleCount:s.eventTitleCount,eventSources:s.eventSources.size,broadCount:s.broadCount,broadTitleCount:s.broadTitleCount,broadSources:s.broadSources.size,latest:s.latest||null};return out;
}
async function collectGlobalPolitics(active,eventTitle='',eventKeywords=[]){
  ...
  specs.push({q:'국회 정치 when:1d',tag:'broad'},{q:'더불어민주당 국민의힘 when:1d',tag:'broad'},{q:'지방정부 시장 도지사 구청장 군수 when:1d',tag:'broad'});
  const warnings=[],all=[];
...
async function fetchX(member){
  ...
  // 이름 단독 X 검색은 배우·기업인 등 동명이인을 섞을 수 있으므로 직책/지역 단서를 항상 결합한다.
  const local=['metro','local'].includes(String(member?.entityType||'assembly'));
  const role=titleTerm(member),place=local?String(member?.jurisdiction||member?.region||'').trim():'';
  const dup=member?.ambiguousName?disambiguationTerms(member).slice(0,2).join(' '):'';
  const query=[`"${member.name}"`,role?`"${role}"`:'',place?`"${place}"`:'',dup,'lang:ko','-is:retweet'].filter(Boolean).join(' ');
...
async function collectEnrichmentMember(member,keywords=[],globalSignals={},options={}){
  ...
  if(options.allowBigKinds!==false&&bigKindsCredentials().configured){out.bigKinds=await collectBigKinds(member);health.bigKinds={state:out.bigKinds?.observation?.state==='MISSING'?'ERROR':'OK',detail:out.bigKinds?.observation?.detail||''};}
...
function preliminaryHeat(signal){
  ...
  const surf=(c.daumSurface?.surfaceHits||0);
