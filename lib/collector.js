...
function memberSearchQuery(member,{quoted=true}={}){const name=quoted?`\"${member.name}\"`:member.name;const clue=member?.ambiguousName?disambiguationTerms(member)[0]:'';return [name,clue].filter(Boolean).join(' ');}
...
async function fetchKakao(member,type){
  ...
  const query=`${memberSearchQuery(member,{quoted:false})} 국회의원`;
...
async function fetchYouTube(member,keywords=[]){
  ...
  const q=`${memberSearchQuery(member,{quoted:false})} 국회의원`;
...
async function fetchX(member){
  ...
  const query=`"${member.name}"${member?.ambiguousName?' '+disambiguationTerms(member).slice(0,2).join(' '):''} lang:ko -is:retweet`;
