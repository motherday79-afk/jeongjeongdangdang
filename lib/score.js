...
function computeSnapshot(roster,signals,context={},previous=null,trafficSignals={}){
  ...
  const activeRows=scored.filter(x=>Number(x.id)!==300),top30=[...activeRows].sort((a,b)=>a.overallRank-b.overallRank).slice(0,30),namePulseObserved=activeRows.filter(x=>['OBSERVED','ZERO'].includes(x.signal?.channels?.namePulse?.observation?.state)).length,namePulseSignal=activeRows.filter(x=>Number(x.signal?.channels?.namePulse?.monthlyTotalQcCnt||0)>0).length,bigKindsObserved=activeRows.filter(x=>['OBSERVED','ZERO'].includes(x.signal?.channels?.bigKinds?.observation?.state)).length;
