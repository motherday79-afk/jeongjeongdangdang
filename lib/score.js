...
function computeSnapshot(roster,signals,context={},previous=null,trafficSignals={}){
  ...
  scored=scored.map((x,i)=>{const display=relativeDisplay(x.components.raw,pools.score,{floor:12,top:98,percentileWeight:.76});const prev=previous?.members?.find?.(p=>p.name===x.member.name),changeRefresh=comparisonCompatible&&Number.isFinite(prev?.rank)?prev.rank-(i+1):null,searchIndex=relativeDisplay(x.components.searchInterest,pools.search),mediaIndex=relativeDisplay(x.components.news,pools.news),metrics=[makeMetric('대중 관심도',searchIndex),makeMetric('뉴스 급상승',mediaIndex),makeMetric('포털·SNS 확산',relativeDisplay(x.components.portalSocial,pools.social)),makeMetric('이슈 중심성',relativeDisplay(x.components.event,pools.event)),makeMetric('신선도·가속도',relativeDisplay(x.components.freshAccel,pools.fresh))],channels=x.signal.channels||{},sourceBadges=[];
  ...
  const sourceSummary=Object.entries(configured).filter(([,on])=>on).map(([k])=>k);return {version:'NOW Rank v1.13 Ranking Views + Refresh Delta',modelVersion:'name-pulse-v1',displayScale:'relative-v1.6.1',comparisonCompatible,timestamp:new Date().toISOString(),cadenceHours:6,event:{title:context.eventTitle||'',keywords:context.eventKeywords||[]},configuredSources:configured,sourceSummary,sourceHealth:context.sourceHealth||{},globalNews:{eventArticles:globalNews.eventArticleCount||0,eventSources:globalNews.eventSourceCount||0,broadArticles:globalNews.broadArticleCount||0,broadSources:globalNews.broadSourceCount||0,warnings:globalNews.warnings||[]},detectedIssues:clusters.slice(0,5),quality,members:scored};
}
