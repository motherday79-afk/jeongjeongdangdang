...
const MODEL_VERSION='name-pulse-v1-memberid-v2';
...
function computeSnapshot(roster,signals,context={},previous=null,trafficSignals={}){
  ...
  let scored=active.map(m=>{const k=memberKey(m),sig=signalFor(signals,m)||{},traffic=signalFor(trafficSignals,m)||{},prev=findPreviousMember(previous,m),auto=automaticCentrality(k,clusters),glob=globalEventCentrality(k,globalNews),prevSig=prev?.signal||{},c=computeMember(sig,traffic,{auto,global:glob},prevSig);sig.channels=c.effectiveChannels;const ch=sig.channels||{},tiebreak=(ch.news?.count24||0)*1e-5+(ch.news?.sources24||0)*1e-6-m.id*1e-10;return {member:m,key:k,previous:prev,signal:sig,traffic,components:c,sortScore:c.raw+tiebreak};}).sort((a,b)=>b.sortScore-a.sortScore);
  const pools={
  ...
  scored=scored.map((x,i)=>{const display=relativeDisplay(x.components.raw,pools.score,{floor:12,top:98,percentileWeight:.76});const prev=x.previous,changeRefresh=comparisonCompatible&&Number.isFinite(prev?.rank)?prev.rank-(i+1):null,searchIndex=relativeDisplay(x.components.searchInterest,pools.search),mediaIndex=relativeDisplay(x.components.news,pools.news),metrics=[makeMetric('대중 관심도',searchIndex),makeMetric('뉴스 급상승',mediaIndex),makeMetric('포털·SNS 확산',relativeDisplay(x.components.portalSocial,pools.social)),makeMetric('이슈 중심성',relativeDisplay(x.components.event,pools.event)),makeMetric('신선도·가속도',relativeDisplay(x.components.freshAccel,pools.fresh))],channels=x.signal.channels||{},sourceBadges=[];
    ...
    return {id:x.member.id,memberKey:x.key,name:x.member.name,party:x.member.party,region:x.member.region,constituency:x.member.constituency,rank:i+1,score:display,rawScore:round1(x.components.raw),grade:grade(display),previousRank:Number.isFinite(prev?.rank)?prev.rank:null,changeRefresh,change6h:changeRefresh,searchScore:round1(searchIndex),searchRaw:round1(x.components.searchInterest),mediaScore:round1(mediaIndex),mediaRaw:round1(x.components.news),metrics,sourceCount:x.components.families,evidenceConfidence:round1(x.components.evidenceConfidence),collection:x.components.collection,audit:x.components.audit,sourceBadges,eventLabel:context.eventTitle||'현재 주요 정치 이슈',latest:channels.news?.latest?new Date(channels.news.latest).toISOString():null,signal:{count6:channels.news?.count6||0,count24:channels.news?.count24||0,count7d:channels.news?.count7d||0,sources6:channels.news?.sources6||0,event6:channels.news?.event6||0,headlines:channels.news?.headlines||[],bigKindsHeadlines:channels.bigKinds?.headlines||[],bigKindsRelatedWords:channels.bigKinds?.relatedWords||[],bigKindsIncidents:channels.bigKinds?.incidents||[],channels,traffic:x.traffic,autoEvent:round1(x.components.details.autoEvent),globalEvent:round1(x.components.details.globalEvent),manualEvent:round1(x.components.details.mEvent)}};});
  const searchOrdered=[...scored].sort((a,b)=>(Number(b.searchRaw||0)-Number(a.searchRaw||0))||(Number(b.searchScore||0)-Number(a.searchScore||0))||a.rank-b.rank);
  searchOrdered.forEach((m,i)=>{m.searchRank=i+1;});
  const mediaOrdered=[...scored].sort((a,b)=>(Number(b.mediaRaw||0)-Number(a.mediaRaw||0))||(Number(b.mediaScore||0)-Number(a.mediaScore||0))||a.rank-b.rank);
  mediaOrdered.forEach((m,i)=>{m.mediaRank=i+1;});
  scored.push({id:300,name:'강릉시 국회의원 공석',party:'공석',region:'강원',constituency:'강원 강릉시',rank:300,score:0,rawScore:0,grade:'D',previousRank:300,changeRefresh:0,change6h:0,searchRank:300,searchScore:0,searchRaw:0,mediaRank:300,mediaScore:0,mediaRaw:0,metrics:[['대중 관심도','D',0],['뉴스 급상승','D',0],['포털·SNS 확산','D',0],['이슈 중심성','D',0],['신선도·가속도','D',0]],sourceCount:0,evidenceConfidence:0,sourceBadges:[],eventLabel:'공석',signal:{}});
  ...
  const sourceSummary=Object.entries(configured).filter(([,on])=>on).map(([k])=>k);return {version:'NOW Rank v1.14.8 Roster Identity Hotfix',modelVersion:MODEL_VERSION,rosterVersion:ROSTER_VERSION,displayScale:'relative-v1.6.1',comparisonCompatible,timestamp:new Date().toISOString(),cadenceHours:6,event:{title:context.eventTitle||'',keywords:context.eventKeywords||[]},configuredSources:configured,sourceSummary,sourceHealth:context.sourceHealth||{},globalNews:{eventArticles:globalNews.eventArticleCount||0,eventSources:globalNews.eventSourceCount||0,broadArticles:globalNews.broadArticleCount||0,broadSources:globalNews.broadSourceCount||0,warnings:globalNews.warnings||[]},detectedIssues:clusters.slice(0,5),quality,members:scored};
}
