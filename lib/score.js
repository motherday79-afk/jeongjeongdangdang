...
function sourceFamilyCount(signal={},traffic={}){
  ...
  if((ch.namePulse?.score||0)>0||(ch.naverSearchTrend?.score||0)>0||(ch.googleTrends?.score||0)>0)n++;
...
function memoryFactor(key){if(key==='namePulse')return .96;if(key==='naverSearchTrend')return .84;if(key==='wiki')return .78;if(key==='news')return .58;if(['naverBlogApi','naverCafeApi','naverWebApi','naverSurface','daumSurface','naverView','daumBlog'].includes(key))return .66;return .60;}
function effectiveChannels(current={},previous={}){
  const out={};const keys=new Set([...Object.keys(previous||{}),...Object.keys(current||{}), 'namePulse','naverSearchTrend','news','naverBlogApi','naverCafeApi','naverWebApi','wiki','youtube','youtubeHtml','x']);
...
function computeMember(signal={},traffic={},eventSignals={},previousSignal={}){
  ...
  const namePulse=clamp(Number(ch.namePulse?.score||0)),googleTrend=clamp(Number(ch.googleTrends?.score||0)),naverTrend=clamp(Number(ch.naverSearchTrend?.score||0));
  ...
  // Keep rolling monthly demand bounded. `blendAvailable` normally re-normalizes when
  // other sensors are missing; doing that to Name Pulse could make a monthly baseline
  // accidentally become the whole search axis. So Name Pulse owns a hard 40% slice.
  const dynamicSearch=blendAvailable([
    {value:naverTrend,weight:.4167,availability:availability(ch.naverSearchTrend)},
    {value:googleTrend,weight:.1333,availability:availability(ch.googleTrends)},
    {value:wiki.score,weight:.20,availability:availability(ch.wiki)},
    {value:internal.score,weight:.0833,availability:(traffic.count6||traffic.baseline7d)?1:0},
    {value:searchSurface,weight:.1667,availability:Math.max(availability(ch.naverSurface),availability(ch.daumSurface),availability(ch.naverWebApi),availability(ch.wiki))}
  ]);
  const dynamicSearchAvail=Math.max(availability(ch.naverSearchTrend),availability(ch.googleTrends),availability(ch.wiki),(traffic.count6||traffic.baseline7d)?1:0,availability(ch.naverSurface),availability(ch.daumSurface),availability(ch.naverWebApi));
  const pulseAvail=availability(ch.namePulse);
  const searchInterest=clamp(pulseAvail>0 ? .40*namePulse + (dynamicSearchAvail>0?.60*dynamicSearch:0) : dynamicSearch);
  ...
  const searchAvail=Math.max(availability(ch.namePulse),availability(ch.naverSearchTrend),availability(ch.googleTrends),availability(ch.wiki),availability(ch.naverSurface),availability(ch.daumSurface),availability(ch.naverWebApi));
...
function computeSnapshot(roster,signals,context={},previous=null,trafficSignals={}){
  ...
  const top30=scored.slice(0,30),allActive=scored.slice(0,299),namePulseObserved=allActive.filter(x=>['OBSERVED','ZERO'].includes(x.signal?.channels?.namePulse?.observation?.state)).length,trendObserved=allActive.filter(x=>['OBSERVED','ZERO'].includes(x.signal?.channels?.naverSearchTrend?.observation?.state)).length,commonObserved=allActive.filter(x=>['OBSERVED','ZERO'].includes(x.signal?.channels?.naverBlogApi?.observation?.state)&&['OBSERVED','ZERO'].includes(x.signal?.channels?.naverCafeApi?.observation?.state)).length;
  const quality={configuredSources:configured,namePulseCoverage:namePulseObserved,top30MultiSource:top30.filter(x=>x.sourceCount>=2).length,top30ThreePlus:top30.filter(x=>x.sourceCount>=3).length,top30SingleSource:top30.filter(x=>x.sourceCount<=1).length,naverTrendCoverage:trendObserved,naverCommonCoverage:commonObserved,warning:''};if(quality.top30SingleSource>10)quality.warning='상위권 다수가 단일 원천입니다. 이 스냅샷은 게시보다 원천 상태 점검이 우선입니다.';if(configured.naverNamePulse&&namePulseObserved<280)quality.warning=[quality.warning,`NAVER Name Pulse 전수 관측 ${namePulseObserved}/299`].filter(Boolean).join(' · ');if(configured.naverSearchTrend&&trendObserved<280)quality.warning=[quality.warning,`NAVER 검색어 트렌드 전수 관측 ${trendObserved}/299`].filter(Boolean).join(' · ');
  const sourceSummary=Object.entries(configured).filter(([,on])=>on).map(([k])=>k);return {version:'NOW Rank v1.11 Name Pulse Integrated',modelVersion:'name-pulse-v1',displayScale:'relative-v1.6.1',comparisonCompatible,timestamp:new Date().toISOString(),cadenceHours:6,event:{title:context.eventTitle||'',keywords:context.eventKeywords||[]},configuredSources:configured,sourceSummary,sourceHealth:context.sourceHealth||{},globalNews:{eventArticles:globalNews.eventArticleCount||0,eventSources:globalNews.eventSourceCount||0,broadArticles:globalNews.broadArticleCount||0,broadSources:globalNews.broadSourceCount||0,warnings:globalNews.warnings||[]},detectedIssues:clusters.slice(0,5),quality,members:scored};
}
