...
const {memberKey,findPreviousMember,signalFor}=require('./member_key');
const {chooseRankHeadline}=require('./rank_headline');
const MODEL_VERSION='political-entity-now-v2-local-2026';
...
const STOP=new Set('국회의원 의원 국회 시장 도지사 구청장 군수 광역단체장 기초단체장 지방정부 정치 정치권 더불어민주당 민주당 국민의힘 조국혁신당 진보당 개혁신당 무소속 대표 위원장 정부 대통령 대한민국 관련 오늘 최근 대해 위한 통해 하는 했다 한다 있는 없는 기자 뉴스 단독 종합 속보 인터뷰 포토 영상'.split(/\s+/));
...
function computeSnapshot(roster,signals,context={},previous=null,trafficSignals={}){
  const active=roster.filter(x=>Number(x.id)!==300&&x.party!=='공석'),clusters=buildIssueClusters(signals),globalNews=context.globalNews||{};
  const comparisonCompatible=!previous||previous.modelVersion===MODEL_VERSION;
  let ranked=active.map(m=>{const k=memberKey(m),sig=signalFor(signals,m)||{},traffic=signalFor(trafficSignals,m)||{},prev=findPreviousMember(previous,m),auto=automaticCentrality(k,clusters),glob=globalEventCentrality(k,globalNews),prevSig=prev?.signal||{},c=computeMember(sig,traffic,{auto,global:glob},prevSig);sig.channels=c.effectiveChannels;const ch=sig.channels||{},tiebreak=(ch.news?.count24||0)*1e-5+(ch.news?.sources24||0)*1e-6-Number(m.id||0)*1e-10;return {member:m,key:k,previous:prev,signal:sig,traffic,components:c,sortScore:c.raw+tiebreak};}).sort((a,b)=>b.sortScore-a.sortScore);
  const categoryCursor={assembly:0,metro:0,local:0};
  ranked.forEach((x,i)=>{const type=String(x.member.entityType||'assembly');categoryCursor[type]=(categoryCursor[type]||0)+1;x.overallRank=i+1;x.categoryRank=categoryCursor[type];});
  const pools={score:ranked.map(x=>x.components.raw),search:ranked.map(x=>x.components.searchInterest),news:ranked.map(x=>x.components.news),social:ranked.map(x=>x.components.portalSocial),event:ranked.map(x=>x.components.event),fresh:ranked.map(x=>x.components.freshAccel)};
  let scored=ranked.map(x=>{
    const display=relativeDisplay(x.components.raw,pools.score,{floor:12,top:98,percentileWeight:.76}),prev=x.previous;
    const changeRefresh=comparisonCompatible&&Number.isFinite(prev?.rank)?prev.rank-x.categoryRank:null;
    const changeOverallRefresh=comparisonCompatible&&Number.isFinite(prev?.overallRank)?prev.overallRank-x.overallRank:null;
    const searchIndex=relativeDisplay(x.components.searchInterest,pools.search),mediaIndex=relativeDisplay(x.components.news,pools.news);
    const metrics=[makeMetric('대중 관심도',searchIndex),makeMetric('뉴스 급상승',mediaIndex),makeMetric('포털·SNS 확산',relativeDisplay(x.components.portalSocial,pools.social)),makeMetric('이슈 중심성',relativeDisplay(x.components.event,pools.event)),makeMetric('신선도·가속도',relativeDisplay(x.components.freshAccel,pools.fresh))],channels=x.signal.channels||{},sourceBadges=[];
    if((channels.namePulse?.monthlyTotalQcCnt||0)>0)sourceBadges.push('NAVER Name Pulse');if((channels.googleTrends?.score||0)>0)sourceBadges.push('Google Trends');if((channels.wiki?.count7d||0)>0)sourceBadges.push('Wikipedia');if((channels.news?.count6||0)>0)sourceBadges.push('뉴스');if((x.components.details.globalEvent||0)>0)sourceBadges.push('이슈뉴스');if((channels.bigKinds?.count7d||0)>0||(channels.bigKinds?.relatedWords||[]).length)sourceBadges.push('BIG KINDS');if((channels.daumSurface?.surfaceHits||0)>0||(channels.daumBlog?.surfaceHits||0)>0)sourceBadges.push('Daum HTML');if(['web','blog','cafe'].some(k=>(channels[k]?.count6||0)>0))sourceBadges.push('포털API');if((channels.video?.count6||0)>0)sourceBadges.push('동영상');if((channels.youtube?.count6||0)>0)sourceBadges.push('YouTube API');if((channels.youtubeHtml?.count7d||0)>0)sourceBadges.push('YouTube HTML');if((channels.x?.count6||0)>0)sourceBadges.push('X');if((x.traffic.count6||0)>=3)sourceBadges.push('정참시');
    const pickedHeadline=chooseRankHeadline(x.member,x.signal,globalNews,prev,Date.now());
    return {id:x.member.id,memberKey:x.key,entityType:x.member.entityType||'assembly',office:x.member.office||'국회의원',jurisdiction:x.member.jurisdiction||x.member.constituency||'',name:x.member.name,party:x.member.party,region:x.member.region,constituency:x.member.constituency,photoVerified:x.member.photoVerified===true,officialPhoto:x.member.photoVerified===true?x.member.officialPhoto||null:null,officialProfileUrl:x.member.officialProfileUrl||null,photoPolicy:x.member.photoPolicy||'official-only',overallRank:x.overallRank,rank:x.categoryRank,categoryRank:x.categoryRank,score:display,rawScore:round1(x.components.raw),grade:grade(display),previousRank:Number.isFinite(prev?.rank)?prev.rank:null,previousOverallRank:Number.isFinite(prev?.overallRank)?prev.overallRank:null,changeRefresh,changeOverallRefresh,change6h:changeRefresh,searchScore:round1(searchIndex),searchRaw:round1(x.components.searchInterest),mediaScore:round1(mediaIndex),mediaRaw:round1(x.components.news),metrics,sourceCount:x.components.families,evidenceConfidence:round1(x.components.evidenceConfidence),collection:x.components.collection,audit:x.components.audit,sourceBadges,eventLabel:context.eventTitle||'현재 주요 정치 이슈',latest:channels.news?.latest?new Date(channels.news.latest).toISOString():null,rankHeadline:pickedHeadline.headline||'',rankHeadlineSource:pickedHeadline.source||'',rankHeadlineAt:pickedHeadline.ts||null,rankHeadlineKind:pickedHeadline.kind||'none',signal:{count6:channels.news?.count6||0,count24:channels.news?.count24||0,count7d:channels.news?.count7d||0,sources6:channels.news?.sources6||0,event6:channels.news?.event6||0,headlines:channels.news?.headlines||[],headlineCandidates:channels.news?.headlineCandidates||[],bigKindsHeadlines:channels.bigKinds?.headlines||[],bigKindsRelatedWords:channels.bigKinds?.relatedWords||[],bigKindsIncidents:channels.bigKinds?.incidents||[],channels,traffic:x.traffic,autoEvent:round1(x.components.details.autoEvent),globalEvent:round1(x.components.details.globalEvent),manualEvent:round1(x.components.details.mEvent)}};
  });
  const searchOrdered=[...scored].sort((a,b)=>(Number(b.searchRaw||0)-Number(a.searchRaw||0))||(Number(b.searchScore||0)-Number(a.searchScore||0))||a.overallRank-b.overallRank);searchOrdered.forEach((m,i)=>{m.searchRank=i+1;});
  const mediaOrdered=[...scored].sort((a,b)=>(Number(b.mediaRaw||0)-Number(a.mediaRaw||0))||(Number(b.mediaScore||0)-Number(a.mediaScore||0))||a.overallRank-b.overallRank);mediaOrdered.forEach((m,i)=>{m.mediaRank=i+1;});
  const vacancy=roster.find(x=>Number(x.id)===300)||{};
  scored.push({id:300,memberKey:'id:300',entityType:'assembly',office:'국회의원 공석',jurisdiction:vacancy.constituency||'강원 강릉시',name:vacancy.name||'강릉시 국회의원 공석',party:'공석',region:vacancy.region||'강원',constituency:vacancy.constituency||'강원 강릉시',overallRank:null,rank:300,categoryRank:300,score:0,rawScore:0,grade:'D',previousRank:300,changeRefresh:0,change6h:0,searchRank:null,searchScore:0,searchRaw:0,mediaRank:null,mediaScore:0,mediaRaw:0,metrics:[['대중 관심도','D',0],['뉴스 급상승','D',0],['포털·SNS 확산','D',0],['이슈 중심성','D',0],['신선도·가속도','D',0]],sourceCount:0,evidenceConfidence:0,sourceBadges:[],eventLabel:'공석',signal:{}});
  ...
  const activeRows=scored.filter(x=>Number(x.id)!==300),top30=[...activeRows].sort((a,b)=>a.overallRank-b.overallRank).slice(0,30);
  for(const row of top30){if(!row.rankHeadline){row.rankHeadline='최근 주요 뉴스 확인 중';row.rankHeadlineSource='정참시 TOP30 안전표시';row.rankHeadlineKind='top30-fallback';}}
  const namePulseObserved=activeRows.filter(x=>['OBSERVED','ZERO'].includes(x.signal?.channels?.namePulse?.observation?.state)).length,namePulseSignal=activeRows.filter(x=>Number(x.signal?.channels?.namePulse?.monthlyTotalQcCnt||0)>0).length,bigKindsObserved=activeRows.filter(x=>['OBSERVED','ZERO'].includes(x.signal?.channels?.bigKinds?.observation?.state)).length;
  const statusCounts={GOOD:activeRows.filter(x=>x.collection?.status==='GOOD').length,NEUTRAL:activeRows.filter(x=>x.collection?.status==='NEUTRAL').length,POOR:activeRows.filter(x=>x.collection?.status==='POOR').length};
  const categoryCounts={assembly:activeRows.filter(x=>x.entityType==='assembly').length,metro:activeRows.filter(x=>x.entityType==='metro').length,local:activeRows.filter(x=>x.entityType==='local').length};
  const quality={configuredSources:configured,totalActive:activeRows.length,categoryCounts,namePulseCoverage:namePulseObserved,namePulseSignalCoverage:namePulseSignal,bigKindsCoverage:bigKindsObserved,top30MultiSource:top30.filter(x=>x.sourceCount>=2).length,top30ThreePlus:top30.filter(x=>x.sourceCount>=3).length,top30SingleSource:top30.filter(x=>x.sourceCount<=1).length,top30HeadlineCoverage:top30.filter(x=>x.rankHeadline&&x.rankHeadlineKind!=='top30-fallback').length,top30HeadlineFallback:top30.filter(x=>x.rankHeadlineKind==='top30-fallback').length,collectionStatus:statusCounts,warning:''};
  if(quality.top30SingleSource>10)quality.warning='상위권 다수가 단일 원천입니다. 이 스냅샷은 게시보다 원천 상태 점검이 우선입니다.';
  const coverageTarget=Math.floor(activeRows.length*.95);if(configured.naverNamePulse&&namePulseObserved<coverageTarget)quality.warning=[quality.warning,`NAVER Name Pulse 실제 수집 실패 포함 처리 ${namePulseObserved}/${activeRows.length}`].filter(Boolean).join(' · ');
  const sourceSummary=Object.entries(configured).filter(([,on])=>on).map(([k])=>k);
  return {version:'정참시 NOW Rank v2.2.5',modelVersion:MODEL_VERSION,rosterVersion:ROSTER_VERSION,displayScale:'relative-v2.2-all-politics',comparisonCompatible,timestamp:new Date().toISOString(),cadenceHours:6,event:{title:context.eventTitle||'',keywords:context.eventKeywords||[]},configuredSources:configured,sourceSummary,sourceHealth:context.sourceHealth||{},globalNews:{eventArticles:globalNews.eventArticleCount||0,eventSources:globalNews.eventSourceCount||0,broadArticles:globalNews.broadArticleCount||0,broadSources:globalNews.broadSourceCount||0,warnings:globalNews.warnings||[]},detectedIssues:clusters.slice(0,8),quality,members:scored};
}
