...
const MASTER_VERSION='v273-photo-master-2';
const VARIANTS={
  160:{width:160,height:160,quality:76},
  360:{width:360,height:360,quality:80}
};
const POSITIVE_TTL=180*24*3600;
const LOCK_TTL=35;
...
async function saveRecord(rec){await setJSON(key(rec.id,rec.size),rec,POSITIVE_TTL);return rec;}
...
async function buildPhotoMaster(id,{force=false}={}){
  ...
  if(!force){const [a,b]=await Promise.all([getPhotoMaster(n,160),getPhotoMaster(n,360)]);if(a&&b)return {ok:true,cached:true,records:[a,b]};}
  ...
  try{
    ...
    await Promise.all([saveRecord(r160),saveRecord(r360),setJSON(statusKey(n),stat,POSITIVE_TTL),cmd(['HSET',INDEX_KEY,String(n),JSON.stringify(stat)]).catch(()=>{})]);
...
module.exports={MASTER_VERSION,variantSize,getPhotoMaster,getOrBuildPhotoMaster,buildPhotoMaster,invalidatePhotoMaster,photoMasterStatus,fetchOriginal};
