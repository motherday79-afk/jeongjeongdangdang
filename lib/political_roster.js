...
const governmentRaw=require('../data/government_officials.json');
...
const governmentRoster=(Array.isArray(governmentRaw)?governmentRaw:[]).map(x=>({...x,entityType:'government',photoPolicy:x.photoPolicy||'verified-web'}));
...
function governmentOfficials(){return governmentRoster.map(x=>({...x}));}
function photoRoster(){return [...activeRoster(),...governmentOfficials()];}
function findEntity(id){return getAllRoster().find(x=>Number(x.id)===Number(id))||governmentRoster.find(x=>Number(x.id)===Number(id))||null;}
...
module.exports={getAllRoster,activeRoster,assemblyRoster,localLeaders,governmentOfficials,photoRoster,findEntity,counts};
