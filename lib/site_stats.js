  ...
  try{
    ...
    if(x&&Number(x.schemaVersion)>=2&&x.date){
      return {
        ...
        todayBoost:int(x.todayBoost),
        totalBoost:int(x.totalBoost),
        showOnline:x.showOnline!==false,
        showToday:x.showToday!==false,
        showTotal:x.showTotal!==false,
        updatedAt:x.updatedAt||null,
        updatedBy:x.updatedBy||null,
...
async function ensureDailyRollover(now=Date.now(),reason='request'){
  ...
  state={
    ...
    todayBoost:0,
    totalBoost:0,
    showOnline:state.showOnline!==false,
    showToday:state.showToday!==false,
    showTotal:state.showTotal!==false,
...
function applyDisplay(raw,cfg){
  ...
  return {
    ...
    visibility:{
      online:cfg.showOnline!==false,
      today:cfg.showToday!==false,
      total:cfg.showTotal!==false
    }
  };
}
...
async function setDisplayVisibility({online,today,total,updatedBy}={},now=Date.now()){
  const cfg=await ensureDailyRollover(now,'admin-visibility');
  const next={
    ...cfg,
    schemaVersion:2,
    showOnline:online!==false,
    showToday:today!==false,
...
module.exports={
  touchVisitor,readStats,readAdminStats,setDisplayTargets,setDisplayVisibility,resetDisplayTargets,finalizeDaily,
