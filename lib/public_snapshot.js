...
function compactMember(x={}){
  const sig=x.signal||{};
  return {
    id:x.id,memberKey:x.memberKey||(`id:${Number(x.id)}`),entityType:x.entityType||'assembly',office:x.office||'국회의원',jurisdiction:x.jurisdiction||x.constituency||'',name:x.name,party:x.party,region:x.region,constituency:x.constituency,photoVerified:x.photoVerified===true,officialPhoto:x.photoVerified===true?x.officialPhoto||null:null,officialProfileUrl:x.officialProfileUrl||null,photoPolicy:x.photoPolicy||'official-only',
    overallRank:Number.isFinite(Number(x.overallRank))?Number(x.overallRank):null,categoryRank:Number.isFinite(Number(x.categoryRank))?Number(x.categoryRank):Number(x.rank),
    ...
    previousOverallRank:Number.isFinite(x.previousOverallRank)?x.previousOverallRank:null,
    changeRefresh:Number.isFinite(x.changeRefresh)?x.changeRefresh:(Number.isFinite(x.change6h)?x.change6h:null),
    changeOverallRefresh:Number.isFinite(x.changeOverallRefresh)?x.changeOverallRefresh:null,
    ...
    rankHeadline:x.rankHeadline||'',rankHeadlineSource:x.rankHeadlineSource||'',rankHeadlineAt:x.rankHeadlineAt||null,rankHeadlineKind:x.rankHeadlineKind||'',
    ...
    signal:{
      ...
      headlineCandidates:(Array.isArray(sig.headlineCandidates)?sig.headlineCandidates:[]).slice(0,10).map(compactHeadline).filter(Boolean),
...
function publicSnapshot(snap={}){
  return {
    ...
    timestamp:snap.timestamp||null,version:snap.version||null,modelVersion:snap.modelVersion||null,rosterVersion:snap.rosterVersion||null,
