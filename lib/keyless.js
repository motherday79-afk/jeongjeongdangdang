...
function memberSearchText(member){const clue=member?.ambiguousName?disambiguationTerms(member)[0]:'';return [member?.name,'국회의원',clue].filter(Boolean).join(' ');}
...
function memberTrendSignal(member,items=[]){
  const name=normalize(member.name),clues=disambiguationTerms(member).map(normalize);let best=null;
...
async function wikiTitle(member){
  ...
  const clues=disambiguationTerms(member);const scored=rows.map(x=>{const t=x.title||'',sn=cleanHtml(x.snippet||''),all=`${t} ${sn}`;let s=0;if(t===member.name)s+=10;if(t.includes(member.name))s+=5;if(/정치인|국회의원|대한민국/.test(all))s+=4;if(member.ambiguousName){const hit=clues.some(k=>k&&all.includes(k));if(hit)s+=10;else s-=20;}return {title:t,s};}).sort((a,b)=>b.s-a.s);
