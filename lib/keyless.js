...
const {searchQualifier,roleAliases}=require('./political_entity');
...
function memberSearchText(member){return searchQualifier(member);}
...
async function scrapeSearchPage(member,keywords,kind,permission,variant='all'){
  let cfg;
  const q=encodeURIComponent(memberSearchText(member));
  if(kind==='naver'){
    cfg=variant==='view'
...
function memberTrendSignal(member,items=[]){
  const name=normalize(member.name),clues=disambiguationTerms(member).map(normalize);
  const local=['metro','local'].includes(String(member?.entityType||'assembly'));
  const identityClues=local?[...roleAliases(member),member?.jurisdiction,member?.region].map(normalize).filter(x=>x.length>=2):[];let best=null;
  for(const x of items){
    if(!x.text.includes(name))continue;
    // 지방단체장은 Google Trends에서도 이름만으로 귀속하지 않는다. 직책/관할 단서가 함께 있어야 한다.
    if(local && !identityClues.some(k=>k&&x.text.includes(k)))continue;
    if(member.ambiguousName && !clues.some(k=>k&&x.text.includes(k)))continue;
...
async function wikiTitle(member){
  const q=encodeURIComponent(memberSearchText(member)),url=`https://ko.wikipedia.org/w/api.php?action=query&list=search&format=json&utf8=1&srlimit=5&srsearch=${q}`;
  ...
  const clues=disambiguationTerms(member);const scored=rows.map(x=>{const t=x.title||'',sn=cleanHtml(x.snippet||''),all=`${t} ${sn}`;let s=0;if(t===member.name)s+=10;if(t.includes(member.name))s+=5;if(/정치인|국회의원|시장|도지사|구청장|군수|지방정부|대한민국/.test(all))s+=4;if(roleAliases(member).some(r=>r&&all.includes(r)))s+=5;if(member.ambiguousName){const hit=clues.some(k=>k&&all.includes(k));if(hit)s+=10;else s-=20;}return {title:t,s};}).sort((a,b)=>b.s-a.s);
...
async function scrapeYouTube(member,keywords=[],permission){
  const q=encodeURIComponent(memberSearchText(member)),url=`https://www.youtube.com/results?search_query=${q}&sp=CAI%253D`;
...
async function collectGlobalKeyless(active=[]){
  const [daumRobots,youtubeRobots,trends]=await Promise.all([checkRobots('https://search.daum.net','/search'),checkRobots('https://www.youtube.com','/results'),fetchGoogleTrendsKR()]);
  const trendSignals={};for(const m of active)trendSignals[memberKey(m)]=memberTrendSignal(m,trends.items||[]);
  const advisory=x=>x.allowed===true?'READY':x.allowed===false?'ADVISORY_DISALLOW':'ADVISORY_UNKNOWN';
  return {permissions:{daumSearch:daumRobots,youtubeSearch:youtubeRobots},trends:{signals:trendSignals,count:(trends.items||[]).length,items:trends.items||[]},health:{daumHtml:{state:advisory(daumRobots),detail:daumRobots.state},youtubeHtml:{state:advisory(youtubeRobots),detail:youtubeRobots.state},googleTrends:trends.health}};
}
async function collectKeylessSurface(member,keywords=[],global={}){
  const out={},health={},daum=await scrapeSearchPage(member,keywords,'daum',global.permissions?.daumSearch,'all');
  if(daum.summary)out.daumSurface=daum.summary;health.daumHtml=daum.health;return {channels:out,health};
}
async function collectKeylessEnrichment(member,keywords=[],global={}){
  const out={},health={},rows=await Promise.all([fetchWikiInterest(member),scrapeSearchPage(member,keywords,'daum',global.permissions?.daumSearch,'blog'),scrapeYouTube(member,keywords,global.permissions?.youtubeSearch)]),[wiki,daumBlog,youtube]=rows;
  if(wiki.summary)out.wiki=wiki.summary;health.wiki=wiki.health;if(daumBlog.summary)out.daumBlog=daumBlog.summary;health.daumBlog=daumBlog.health;if(youtube.summary)out.youtubeHtml=youtube.summary;health.youtubeHtml=youtube.health;const trend=global.trends?.signals?.[memberKey(member)];if(trend)out.googleTrends=trend;return {channels:out,health};
}
