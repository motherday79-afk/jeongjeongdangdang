...
async function changeAdminPassword(id,currentPassword,newPassword){
  ...
  return {updatedAt:next.updatedAt,revision:next.revision};
}
async function adminCredentialStatus(){
  const stored=await getStoredCredential();
  return {customPassword:Boolean(stored?.passwordHash),updatedAt:stored?.updatedAt||null,revision:Number(stored?.revision||0)};
}
function sessionCookie(id){
  const token=sign({id,iat:Date.now(),exp:Date.now()+12*60*60*1000});
...
module.exports={
  getSession,requireAdmin,loginAllowed,sessionCookie,clearCookie,
  validateAdminPassword,hashAdminPassword,verifyAdminPassword,
  changeAdminPassword,adminCredentialStatus,ADMIN_CREDENTIAL_KEY
};
