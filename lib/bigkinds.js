...
async function searchMemberNews(member,days=7){
  const argument={query:String(member.name||''),published_at:windowDates(days),sort:[{date:'desc'}],hilight:120,return_from:0,return_size:50,fields:['title','published_at','dateline','enveloped_at','provider','category','category_incident','provider_link_page','tms_raw_stream']};
...
async function relatedWords(member,days=7){
  const argument={query:String(member.name||''),published_at:windowDates(days),provider:[],category:[],category_incident:[],byline:'',provider_subject:[]};
