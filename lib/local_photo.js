...
async function cachePersonPhotoRecord(id,rec){
  ...
  await setJSON(cacheKey(member),{...rec,resolvedAt:rec.resolvedAt||new Date().toISOString(),searchVersion:SEARCH_CACHE_VERSION},CACHE_TTL).catch(()=>{});
...
async function resolvePersonPhoto(id,{force=false,ignoreOverride=false,searchMode='normal',persist=true}={}){
  ...
  const official=member.entityType==='assembly'?await assemblyOfficialPhoto(member).catch(()=>null):(registered||(!ignoreOverride&&override?.url?{...override,verified:true,manual:true}:null));
  ...
  if(official?.url&&!force){
    ...
    if(persist){await setJSON(cacheKey(member),rec,CACHE_TTL).catch(()=>{});await del(negativeKey(member)).catch(()=>{});}return rec;
  }
  ...
  if(persist)await setJSON(negativeKey(member),{missing:true,checkedAt:new Date().toISOString()},NEGATIVE_CACHE_TTL).catch(()=>{});
  return null;
}
const invalidateLocalPhoto=invalidatePersonPhoto;
const resolveLocalPhoto=resolvePersonPhoto;

module.exports={resolvePersonPhoto,invalidatePersonPhoto,clearPersonPhotoOverride,cachePersonPhotoRecord,resolveLocalPhoto,invalidateLocalPhoto,probePhotoRecord,assemblyOfficialPhoto,findImageNearIdentity,findProfileLinkNearIdentity,associationPhoto,webSearchPhoto,parseBingCandidates,parseImageSize,searchQueries,contextScore};
