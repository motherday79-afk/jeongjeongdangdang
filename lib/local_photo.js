...
async function resolveLocalPhoto(id,{force=false}={}){
  ...
  // 공식 협의회 이미지도 후보에 포함하지만, 누락/저해상도 문제를 해결하기 위해
  // 정확한 이름+직위+지역으로 검색한 고해상도 후보를 함께 비교합니다.
  const [assoc,web]=await Promise.all([associationPhoto(member).catch(()=>null),webSearchPhoto(member).catch(()=>null)]);
  let found=web||null;
  if(assoc){
    const assocCandidate=await probeImage({imageUrl:assoc.url,pageUrl:assoc.profileUrl,title:`${member.name} ${member.office} ${member.jurisdiction}`,snippet:assoc.source,provider:'official-association'});
    const assocScore=candidateScore(assocCandidate,member)+20;
    const webScore=web?candidateScore({imageUrl:web.url,pageUrl:web.profileUrl,title:`${member.name} ${member.office}`,snippet:web.source,width:web.width,height:web.height,verifiedContext:true},member):-999;
    if(assocScore>=webScore&&Number(assocCandidate.width)>=500&&Number(assocCandidate.height)>=500){
      found={url:assoc.url,profileUrl:assoc.profileUrl,source:assoc.source,width:assocCandidate.width||null,height:assocCandidate.height||null,verified:true};
    }
  }
  if(found?.url){
...
module.exports={resolveLocalPhoto,invalidateLocalPhoto,findImageNearIdentity,associationPhoto,webSearchPhoto,parseBingCandidates,parseImageSize,searchQueries,contextScore};
