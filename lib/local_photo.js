...
// v2.2.23: 기존 정상 positive cache는 유지하되, 대량 실패를 고착시킨 negative cache만 새 세대로 분리합니다.
const NEGATIVE_CACHE_VERSION='v6-photo-safe';
const REGIONS={
...
function searchQueries(member){
  const name=String(member?.name||'').trim(),office=String(member?.office||'').trim(),place=shortPlace(member),j=String(member?.jurisdiction||'').trim(),generic=genericOffice(member),type=String(member?.entityType||'');
  if(type==='government')return unique([
    `${name} ${j} ${office} 공식 프로필 사진`,
    `${name} ${j} ${office} 사진`,
    `${name} ${j} 장관 프로필`,
    `${name} 대한민국 정부 ${office}`,
    `${name} ${j} 인물사진`,
...
function contextScore(text,member){
  ...
  if(BAD_CONTEXT_WORDS.test(raw)&&!/시장|군수|구청장|도지사|정치|당선|정부|국무총리|부총리|장관|차관|부처/.test(raw))score-=40;
  if(/시장|군수|구청장|도지사|정치|당선|지방선거|대한민국 정부|국무총리|부총리|장관|차관/.test(raw))score+=6;
...
function findProfileLinkNearIdentity(html,member,base){
  const name=String(member?.name||'').trim();if(!name)return null;
  const anchor=/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;let m,best=null;
  while((m=anchor.exec(html))){const text=cleanText(m[2]);if(!text.includes(name))continue;const url=absUrl(m[1],base);if(!url)continue;const score=(/gsp_pg=view|gsp_uid=/i.test(url)?40:0)+contextScore(text,member);if(!best||score>best.score)best={url,score};}
  if(best?.url)return best.url;
  let from=0;while(true){const idx=html.indexOf(name,from);if(idx<0)break;from=idx+name.length;const chunk=html.slice(Math.max(0,idx-1800),Math.min(html.length,idx+1800));const mm=chunk.match(/(?:https?:\/\/[^"'\s<>]+)?\/?state\/member\.php\?[^"'<>\s]*gsp_(?:uid|pg)=[^"'<>\s]*/i)||chunk.match(/member\.php\?[^"'<>\s]*gsp_uid=\d+[^"'<>\s]*/i);if(mm){const u=absUrl(htmlDecode(mm[0]),base);if(u)return u;}}
  return null;
}
...
async function associationPhoto(member){
  if(member.entityType==='local'){
    ...
    for(const code of codes){
      ...
      try{
        const html=await fetchText(url);const detail=findProfileLinkNearIdentity(html,member,url);
        if(detail){try{const dh=await fetchText(detail);const di=findImageNearIdentity(dh,member,detail);if(di)return {url:di,profileUrl:detail,source:'대한민국시장군수구청장협의회 회원소개 · 상세 프로필'};}catch(_){} }
        const image=findImageNearIdentity(html,member,url);if(image)return {url:image,profileUrl:url,source:'대한민국시장군수구청장협의회 회원소개'};
      }catch(_){}
    }
  }
...
async function officialProfilePagePhoto(member){
  const profile=String(member?.officialProfileUrl||'').trim();
  if(!profile||!isHttpUrl(profile))return null;
  try{
    const html=await fetchText(profile,7500);
    let image=findImageNearIdentity(html,member,profile);
    if(!image){
      // 일부 장관실 페이지는 이미지 alt 주변에 이름 대신 "장관 프로필"만 둡니다.
      const re=/<img\b[^>]*(?:alt|title)=["'][^"']*(?:장관|총리|프로필|minister)[^"']*["'][^>]*(?:src|data-src)=["']([^"']+)["'][^>]*>/gi;
      let m;while((m=re.exec(html))){const u=absUrl(m[1],profile);if(u&&!BAD_IMAGE_WORDS.test(u)){image=u;break;}}
      if(!image){
        const re2=/<img\b[^>]*(?:src|data-src)=["']([^"']+)["'][^>]*(?:alt|title)=["'][^"']*(?:장관|총리|프로필|minister)[^"']*["'][^>]*>/gi;
        while((m=re2.exec(html))){const u=absUrl(m[1],profile);if(u&&!BAD_IMAGE_WORDS.test(u)){image=u;break;}}
      }
    }
    if(!image)return null;
    const probed=await probeImage({imageUrl:image,pageUrl:profile,title:`${member.name} ${member.office} ${member.jurisdiction||''}`,snippet:`${member.name} ${member.office} 공식 프로필`,provider:'government-official-profile'}).catch(()=>null);
    if(!probed?.imageUrl)return null;
    if(Number(probed.width||0)<300||Number(probed.height||0)<300)return null;
    return {url:probed.imageUrl,profileUrl:profile,source:`${member.jurisdiction||'대한민국 정부'} 공식 프로필`,width:probed.width||null,height:probed.height||null,verified:true,official:true};
  }catch(_){return null;}
}
...
async function bingImageSearch(member,{mode='normal'}={}){
  const qs=searchQueries(member).slice(0,mode==='repair'?6:3);
  const fetchOne=async query=>{
    const urls=[
      `https://www.bing.com/images/search?q=${encodeURIComponent(query)}&form=HDRSC3&first=1&tsc=ImageBasicHover`,
      `https://www.bing.com/images/async?q=${encodeURIComponent(query)}&first=1&count=35&cw=1177&ch=781&relp=35&tsc=ImageBasicHover`
    ];
    for(const url of urls){
      try{const html=await fetchText(url,mode==='repair'?8000:6500);const rows=parseBingCandidates(html,query).slice(0,mode==='repair'?40:28);if(rows.length)return rows;}catch(_){}
    }
    return [];
  };
  if(mode!=='repair')return (await Promise.all(qs.map(fetchOne))).flat();
  const out=[];
  for(const query of qs){const rows=await fetchOne(query);out.push(...rows);if(rows.length>=12)break;await sleep(240);}
  return out;
}
async function kakaoImageSearch(member,{mode='normal'}={}){
  const key=process.env.KAKAO_REST_API_KEY;if(!key)return [];
  const qs=searchQueries(member).slice(0,mode==='repair'?5:3);
  const fetchOne=async query=>{
    const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),mode==='repair'?8000:6000);
    try{
  ...
  if(mode!=='repair')return (await Promise.all(qs.map(fetchOne))).flat();
  const out=[];for(const query of qs){out.push(...await fetchOne(query));if(out.length>=18)break;await sleep(180);}return out;
}
...
async function webSearchPhoto(member,{mode='normal'}={}){
  let kakao=[],bing=[];
  if(mode==='repair'){
    kakao=await kakaoImageSearch(member,{mode}).catch(()=>[]);
    bing=await bingImageSearch(member,{mode}).catch(()=>[]);
  }else{
    [kakao,bing]=await Promise.all([kakaoImageSearch(member,{mode}),bingImageSearch(member,{mode})]);
  }
  ...
  const limit=mode==='repair'?14:8,checked=[];
  if(mode==='repair'){
    for(const c of candidates.slice(0,limit)){checked.push(await probeImage(await verifyCandidateContext(c,member)));if(checked.some(x=>x.verifiedContext&&Number(x.width)>=700&&Number(x.height)>=700))break;}
  }else checked.push(...await Promise.all(candidates.slice(0,limit).map(async c=>probeImage(await verifyCandidateContext(c,member)))));
  let verified=checked.filter(c=>c.verifiedContext);
  if(!verified.length)verified=checked.filter(c=>contextScore(`${c.title||''} ${c.snippet||''}`,member)>=28&&sourceHostScore(c.pageUrl||c.imageUrl)>=16);
...
function lkgKey(member){return `jjdd:local-photo:last-good:${member.id}:v1`; }
function negativeKey(member){return `jjdd:local-photo:negative:${member.id}:${NEGATIVE_CACHE_VERSION}`;}
...
async function clearPersonPhotoOverride(id){const member=findEntity(id);if(!member)return false;await del(`jjdd:local-photo:override:${member.id}`).catch(()=>{});return true;}
async function cachePersonPhotoRecord(id,rec){
  const member=findEntity(id);if(!member||!rec?.url)return false;
  const saved={...rec,resolvedAt:rec.resolvedAt||new Date().toISOString(),searchVersion:SEARCH_CACHE_VERSION};
  await Promise.all([setJSON(cacheKey(member),saved,CACHE_TTL).catch(()=>{}),setJSON(lkgKey(member),saved).catch(()=>{})]);
  await del(negativeKey(member)).catch(()=>{});return true;
}
async function resolvePersonPhotoFast(id,{ignoreOverride=false}={}){
  ...
  if(!ignoreOverride&&override?.url&&/^https?:\/\//i.test(override.url))return {...override,verified:true,manual:true};
  const registered=(member.photoVerified===true&&member.officialPhoto)?{url:member.officialPhoto,profileUrl:member.officialProfileUrl||null,source:member.entityType==='assembly'?'등록된 국회 공식 프로필':member.entityType==='government'?'대한민국 정부 공식 프로필':'등록된 지방정부 공식 프로필',verified:true,official:true}:null;
  if(registered)return registered;
  const cached=await getJSON(cacheKey(member)).catch(()=>null);if(cached?.url)return cached;
  const lkg=await getJSON(lkgKey(member)).catch(()=>null);if(lkg?.url)return {...lkg,lastKnownGood:true};
  return null;
}

async function resolvePersonPhoto(id,{force=false,ignoreOverride=false,searchMode='normal',persist=true}={}){
  const member=findEntity(id);if(!member||Number(member.id)===300||String(member.party||'')==='공석')return null;
  const override=await getJSON(`jjdd:local-photo:override:${member.id}`).catch(()=>null);
  if(!ignoreOverride&&override?.url&&/^https?:\/\//i.test(override.url))return {...override,verified:true,manual:true};
  const registered=(member.photoVerified===true&&member.officialPhoto)?{url:member.officialPhoto,profileUrl:member.officialProfileUrl||null,source:member.entityType==='assembly'?'등록된 국회 공식 프로필':member.entityType==='government'?'대한민국 정부 공식 프로필':'등록된 지방정부 공식 프로필',verified:true,official:true}:null;
  ...
  if(!force){
    const cached=await getJSON(cacheKey(member)).catch(()=>null);if(cached?.url){setJSON(lkgKey(member),cached).catch(()=>{});return cached;}
    // TTL이 지난 뒤 외부 소스가 잠시 장애여도 마지막 정상 사진은 영구 LKG에서 복구합니다.
    const lkg=await getJSON(lkgKey(member)).catch(()=>null);if(lkg?.url)return {...lkg,lastKnownGood:true};
    const neg=await getJSON(negativeKey(member)).catch(()=>null);if(neg?.missing)return null;
  }

  let official=member.entityType==='assembly'?await assemblyOfficialPhoto(member).catch(()=>null):(registered||(!ignoreOverride&&override?.url?{...override,verified:true,manual:true}:null));
  // 정부 주요인사는 등록된 원본 URL이 없더라도 공식 장관실/부처 프로필 페이지에서 먼저 복구합니다.
  if(!official&&member.entityType==='government'&&member.officialProfileUrl){
    official=await officialProfilePagePhoto(member).catch(()=>null);
  }
  // 일반 화면 진입에서는 공식사진을 가장 먼저 사용해 불필요한 외부 검색을 막습니다.
  if(official?.url&&!force){
    const rec={...official,resolvedAt:new Date().toISOString(),searchVersion:SEARCH_CACHE_VERSION};
    if(persist)await cachePersonPhotoRecord(member.id,rec);return rec;
  }
  // v2.2.23 fail-safe: 국회의원은 서버 국회 명단 확보가 순간 실패했다고 해서 Bing/Wikidata 검색으로 추정하거나
  // '사진 없음' negative cache를 만들지 않습니다. 브라우저의 국회 공식 API 비상경로가 이어받습니다.
  if(member.entityType==='assembly'&&!force&&!official?.url)return null;

  // 강제 재검수 또는 지방/정부 인물 처리에서는 이름+직위+지역 검색 후보와 공식 후보를 함께 비교합니다.
  const [assoc,web]=await Promise.all([
    ...
    webSearchPhoto(member,{mode:searchMode}).catch(()=>null)
  ]);
  ...
  // 국회의원/정부 주요인사의 일시적 외부장애를 '사진 없음'으로 고착시키지 않습니다.
  if(persist&&['metro','local'].includes(member.entityType))await setJSON(negativeKey(member),{missing:true,checkedAt:new Date().toISOString()},NEGATIVE_CACHE_TTL).catch(()=>{});
  return null;
}
const invalidateLocalPhoto=invalidatePersonPhoto;
const resolveLocalPhoto=resolvePersonPhoto;

module.exports={resolvePersonPhoto,resolvePersonPhotoFast,invalidatePersonPhoto,clearPersonPhotoOverride,cachePersonPhotoRecord,resolveLocalPhoto,invalidateLocalPhoto,probePhotoRecord,assemblyOfficialPhoto,officialProfilePagePhoto,findImageNearIdentity,findProfileLinkNearIdentity,associationPhoto,webSearchPhoto,parseBingCandidates,parseImageSize,searchQueries,contextScore,SEARCH_CACHE_VERSION,NEGATIVE_CACHE_VERSION};
