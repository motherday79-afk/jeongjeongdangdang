...
const PRESIDENT_PAGE_KEY='jjdd:site:president-page:v1';
...
const DEFAULT_PRESIDENT_PAGE={
  enabled:true,
  eyebrow:'PRESIDENT OF THE REPUBLIC OF KOREA',
  overview:'대통령의 약속과 현 정부의 국정 방향, 그리고 국무총리·주요 장관의 조직 구성을 한 화면에서 확인합니다.',
  agenda:[
    {title:'민생과 경제 회복',body:'가계와 소상공인의 부담을 낮추고 내수 회복과 지속 가능한 성장을 함께 추진합니다.'},
    {title:'AI · 첨단산업 성장',body:'AI·반도체·첨단산업 투자를 국가 성장동력으로 연결하고 산업 경쟁력을 높이는 방향입니다.'},
    {title:'균형성장과 지역 주도',body:'수도권 집중을 완화하고 지역이 성장의 주체가 되는 균형발전 전략을 추진합니다.'},
    {title:'국익 중심 실용외교',body:'한반도 평화와 안보를 바탕으로 국익을 최우선에 둔 실용 외교를 확대합니다.'},
    {title:'국민주권 · 투명한 국정',body:'국민 참여와 소통을 확대하고 국정 운영 과정의 공개성과 투명성을 높이는 방향입니다.'}
  ],
  direction:[
    {title:'모두의 성장',body:'성장의 성과가 특정 집단에 집중되지 않고 국민의 삶으로 이어지는 성장 구조'},
    {title:'지방 주도 성장',body:'지역의 산업·인재·생활 기반을 강화해 수도권과 지역이 함께 성장하는 구조'},
    {title:'안전 · 지속가능 성장',body:'사회 안전과 기후·에너지 전환을 장기 성장 전략과 연결'},
    {title:'문화가 이끄는 성장',body:'K-콘텐츠와 문화산업을 국가 경쟁력과 지역경제의 새로운 축으로 확대'},
    {title:'평화가 뒷받침하는 성장',body:'안정적인 안보와 한반도 평화를 경제·외교 성장의 기반으로 구축'}
  ],
  sourceLabel:'중앙선거관리위원회 당선인 공약 · 대통령 공식 국정발언 기반',
  updatedAt:null,updatedBy:null,schemaVersion:1
};
...
function cleanPolicyItems(items,max=6){
  return (Array.isArray(items)?items:[]).slice(0,max).map(x=>({title:clean(x?.title,70),body:clean(x?.body,360)})).filter(x=>x.title&&x.body);
}
async function getPresidentPage(){
  const stored=await getJSON(PRESIDENT_PAGE_KEY).catch(()=>null);
  const base={...DEFAULT_PRESIDENT_PAGE,...(stored||{})};
  base.agenda=cleanPolicyItems(stored?.agenda||DEFAULT_PRESIDENT_PAGE.agenda,6);
  base.direction=cleanPolicyItems(stored?.direction||DEFAULT_PRESIDENT_PAGE.direction,6);
  return base;
}
async function savePresidentPage(input,admin){
  const current=await getPresidentPage();
  const agenda=cleanPolicyItems(input?.agenda,6),direction=cleanPolicyItems(input?.direction,6);
  if(!agenda.length)throw new Error('대통령 공약·약속 항목을 1개 이상 입력해주세요.');
  if(!direction.length)throw new Error('현 정부 방향 항목을 1개 이상 입력해주세요.');
  const next={...current,enabled:input?.enabled!==false,eyebrow:clean(input?.eyebrow||current.eyebrow,90),overview:clean(input?.overview||current.overview,500),agenda,direction,sourceLabel:clean(input?.sourceLabel||current.sourceLabel,180),updatedAt:new Date().toISOString(),updatedBy:String(admin?.id||'admin'),schemaVersion:1};
  await setJSON(PRESIDENT_PAGE_KEY,next);return next;
}
...
async function editSurveyInPlace(input,admin){
  const current=await getSurvey();
  const title=clean(input?.title,80),subtitle=clean(input?.subtitle,180),options=cleanOptions(input?.options);
  if(title.length<2)throw new Error('설문 제목을 2자 이상 입력해주세요.');
  if(options.length<2||options.length>5)throw new Error('설문 문항은 2개 이상 5개 이하로 입력해주세요.');
  const next={...current,fingerprint:fingerprintSurvey(title,subtitle,options),active:input?.active!==false,title,subtitle,options,updatedAt:new Date().toISOString(),updatedBy:String(admin?.id||'admin'),schemaVersion:1};
  await setJSON(SURVEY_KEY,next);
  return next;
}
...
module.exports={DEFAULT_NOW_ISSUE,DEFAULT_RAPID_RISE,DEFAULT_PRESIDENT_PAGE,DEFAULT_SURVEY,getPresidentPage,savePresidentPage,getNowIssue,saveNowIssue,getRapidRise,saveRapidRise,getSurvey,saveSurvey,editSurveyInPlace,surveyStats,voteSurvey,surveyModeration,recountSurvey,invalidateSurveyVote,restoreSurveyVote,invalidateSurveyOption,setSurveyAdjustment,clearSurveyAdjustments,resetSurveyResponses};
